...
- [x] Audit : **sous-détection server-side sur custom-domain corrigée** (2026-06-18, après faux négatif sur digitalix.xyz lui-même) :
  ...
  - **Constat vérifié en live (Playwright, 2026-06-18)** : digitalix.xyz **EST bien server-side** — les hits GA4 **consentis** partent vers `dgx.digitalix.xyz` (marqueurs `sst.*` Stape, payload base64). Le ping pré-consentement (cookieless, gcs=G100) part lui direct vers `region1.google-analytics.com` (comportement Consent Mode). Le scanner ratait le hit server-side car **obfusqué** (pas de `/g/collect` en clair). FPID non confirmé (à vérifier : toggle server-managed cookies du client GA4 serveur).
