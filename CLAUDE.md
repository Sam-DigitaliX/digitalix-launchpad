...
- [x] Audit : **scanner présenté comme un vrai navigateur** (2026-06-18) : `fetcher.ts` masque `navigator.webdriver` + `--disable-blink-features=AutomationControlled` + UA Chrome 140. But : que la bot detection (Stape…) serve l'expérience représentative. **Constat** : insuffisant sur digitalix.xyz (Stape bloque vraisemblablement au niveau IP datacenter du VPS) → le scanner ne reçoit toujours pas le FPID. Le FPID reste **non observable** par un scanner bot ; d'où le reframe sGTM ci-dessous.
- [x] Audit : **précision pass 2** (2026-06-18, faux positifs/négatifs sur le propre site) :
  - **sGTM reframe** : `maturityLevel = 2` aussi quand la collecte GA4 est routée server-side vers le domaine first-party (`serverSideCollect`), pas seulement quand FPID est capté. Le routage server-side EST l'accomplissement niveau 2 ; le mode cookie (FPID) est un raffinement **non observable au scan** (bot detection). Message « Server-side confirmé… mode cookie à confirmer en DevTools » au lieu du faux « JS-managed niveau 1 ». On n'infère plus « JS-managed » depuis `_ga` (qui cohabite toujours avec FPID).
  - **Pondération** : Performance 15%→**10%** (reversé au Server-Side 25%→30%). La perf n'est pas du tracking pur (`index.ts`).
  - **Pré-consentement** (`pre-consent-violations.ts`) : un ping Google `collect` avec `gcs=G100` (Consent Mode v2, ad+analytics denied, cookieless, anonyme) n'est **plus** compté comme violation RGPD (modélisation tolérée CNIL). Seuls cookies/hits identifiants comptent.
  - **Post-refus** (`post-reject-violations.ts`) : nettoyage du test `gcs` bancal (`includes('11')`) → `G100` = anonymisé conforme, tout autre `G1xx` = storage granted malgré refus = violation.
  - **Détection Magento** (`fetcher.ts`) : `scripts.some(s => s.includes('mage/'))` matchait `image/` (sous-chaîne !) → faux Magento sur tout site avec des assets `/image/`. Corrigé → marqueurs spécifiques (`/mage/`, `Magento_`, `/static/frontend/`, `mage/cookies`).
  - **Recommandations** (`AuditResults.tsx`) : titres sans verbe imposé (suppression de `ACTION_PREFIXES`) → titre = nom du check, l'action contextuelle reste dans la description (fini « Installer une CMP » quand la CMP existe mais manque un bouton refuser).
  - CLS : pas un bug identifié — dé-pondéré via Performance 10% (à re-vérifier si KO sur un audit précis).
- [x] Audit : **cards pré-consentement / post-refus passées en informatif** (2026-06-18) — décision produit après faux positifs persistants. Le scan **ne peut pas certifier** ces points : son environnement (IP datacenter du VPS) reçoit un **default de consentement géo `granted`** → il « voit » du tracking avant consentement (`gcs=granted`, `_ga` posé) qu'un vrai visiteur EU (`denied`) n'a jamais. Donc :
  - Les 2 checks (`pre-consent-violations`, `post-reject-violations`) deviennent **informatifs** (`info`) : **décodage pédagogique du `gcs`** (avant / après accept / après refus) via `consent-signals.ts` (`decodeGcs` : `G100` = pub+analytics refusés/anonymisé, `G111` = tout accordé…) + `gcd` affiché **brut** (encodage interne Google, non interprété) + interprétation **prudente sans affirmation** (« reflète l'environnement de scan, default réel = géo du visiteur, à confirmer en DevTools EU »).
  - **Filet** : un `warning` n'est rendu QUE dans le cas flagrant (tracking identifiant **ET aucun Consent Mode détecté**).
  - **Extraction `gcs`/`gcd` corrigée** (`fetcher.ts`) : décode le **base64** des hits obfusqués (custom loader Stape) → fini le `gcsValues: []` sur les setups server-side.
  - Frontend (`AuditResults.tsx`) : titres « Consentement — signaux avant/après refus » (plus « Violations »).
